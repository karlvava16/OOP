...
void Tree::Insert(Node* newNode)
{
    ...
    while (node != nullptr)
    {
        ...
        if (strcmp(newNode->m_english.c_str(), node->m_english.c_str()) < 0)
        {
            node = node->m_left;
        }
        else
    ...
    else if (strcmp(newNode->m_english.c_str(), temp->m_english.c_str()) < 0)
    {
        temp->m_left = newNode;
    }
    else
...
void Tree::Print(const Node* node) const
{
    if (node != nullptr)
    {
        ...
        cout << node->m_english << '\t' << node->m_russian << '\n';
...
void Tree::Remove(Node* removeNode)
{
    if (removeNode != nullptr)
    {
        ...
        if (temp != removeNode)
        {
            ...
            removeNode->m_english = temp->m_english;
            removeNode->m_russian = temp->m_russian;
        }
...
Node* Tree::Search(Node* node, const char* key) const
{
    ...
    while (node != nullptr && strcmp(key, node->m_english.c_str()) != 0)
    {
        if (strcmp(key, node->m_english.c_str()) < 0)
        {
            node = node->m_left;
        }
        else
